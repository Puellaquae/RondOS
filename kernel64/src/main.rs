...
extern "C" fn after_phase2() -> ! {
    ...
    // ------------------------------------------------------------------ P1
    // Real user programs: ELF64 images built by the `user/` workspace for the
    // `x86_64-rondos` target, shipped in the boot tar the UEFI stub loaded.
    test_user_images();
