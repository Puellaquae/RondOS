...
const _: () = {
    ...
    // Discriminants are the contract: pin every one of them (and the reverse
    // mapping used by the kernel dispatcher).
    assert!(SyscallId::Info as u32 == 0x00);
    assert!(SyscallId::Exit as u32 == 0x10);
    assert!(SyscallId::ThreadExit as u32 == 0x11);
    assert!(SyscallId::ThreadSpawn as u32 == 0x12);
    assert!(SyscallId::Yield as u32 == 0x13);
    assert!(SyscallId::SleepNs as u32 == 0x14);
    assert!(SyscallId::ClockGettime as u32 == 0x15);
    assert!(SyscallId::Log as u32 == 0x16);
    assert!(SyscallId::Spawn as u32 == 0x20);
    assert!(SyscallId::Wait as u32 == 0x21);
    assert!(SyscallId::ProcStatus as u32 == 0x22);
    assert!(SyscallId::Kill as u32 == 0x23);
    assert!(SyscallId::MemMap as u32 == 0x30);
    assert!(SyscallId::MemUnmap as u32 == 0x31);
    assert!(SyscallId::MemShare as u32 == 0x32);
    assert!(SyscallId::MemMapPhys as u32 == 0x33);
    assert!(SyscallId::ChanCreate as u32 == 0x40);
    assert!(SyscallId::ChanSend as u32 == 0x41);
    assert!(SyscallId::ChanRecv as u32 == 0x42);
    assert!(SyscallId::ChanClose as u32 == 0x43);
    assert!(SyscallId::Open as u32 == 0x50);
    assert!(SyscallId::Read as u32 == 0x51);
    assert!(SyscallId::Write as u32 == 0x52);
    assert!(SyscallId::Seek as u32 == 0x53);
    assert!(SyscallId::Stat as u32 == 0x54);
    assert!(SyscallId::Readdir as u32 == 0x55);
    assert!(SyscallId::Close as u32 == 0x56);
    assert!(SyscallId::Unlink as u32 == 0x57);
    // Status values are frozen too.
    assert!(Status::Ok as u32 == 0);
    assert!(Status::Unsupported as u32 == 1);
    assert!(Status::InvalidArgument as u32 == 2);
    assert!(Status::BadAddress as u32 == 3);
    assert!(Status::BadHandle as u32 == 4);
    assert!(Status::Permission as u32 == 5);
    assert!(Status::NotFound as u32 == 6);
    assert!(Status::OutOfMemory as u32 == 7);
    assert!(Status::NotReady as u32 == 8);
    assert!(Status::Cancelled as u32 == 9);
    assert!(Status::Broken as u32 == 10);
    assert!(Status::Fault as u32 == 11);
    // Object kinds, rights and flag bits.
    assert!(ObjKind::Process as u32 == 1);
    assert!(ObjKind::Memory as u32 == 3);
    assert!(ObjKind::Chan as u32 == 4);
    assert!(ObjKind::File as u32 == 5);
    assert!(ObjKind::Dir as u32 == 6);
    assert!(ObjKind::Device as u32 == 7);
    assert!(rights::READ == 1);
    assert!(rights::WRITE == 2);
    assert!(rights::SHARE == 1 << 4);
    assert!(rights::MAP == 1 << 3);
    assert!(mem_flags::READ == 1);
    assert!(mem_flags::SHARE == 1 << 3);
    assert!(open_flags::CREATE == 1 << 2);
    ...
    assert!(core::mem::offset_of!(Stat, len_bytes) == 16);
    assert!(core::mem::offset_of!(Stat, rights) == 32);
    assert!(core::mem::offset_of!(ExitStatus, kind) == 8);
    assert!(core::mem::offset_of!(ExitStatus, addr) == 32);
    assert!(core::mem::offset_of!(DirEntry, len_bytes) == 16);
    assert!(core::mem::offset_of!(DirEntry, name_len) == 24);
    assert!(core::mem::offset_of!(CapDesc, handle) == 16);
    assert!(core::mem::offset_of!(StartupBlock, caps) == 72);
    assert!(core::mem::offset_of!(StartupBlock, window) == 88);
