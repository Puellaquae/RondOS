//! `selftest` — user-space test driver.
//!
...
macro_rules! step {
    ($code:expr, $what:expr) => {
        match $what {
            Ok(v) => v,
            Err(e) => {
                println!("selftest: {} failed: {:?}", stringify!($what), e);
                return $code;
            }
        }
    };
...
fn check_fault_child(root: Handle) -> i32 {
    ...
    if w.reason != wait_reason::FAULT {
        println!("selftest: crash child ended with reason {}", w.reason);
        return 13;
    }
...
fn check_killed_child(root: Handle) -> i32 {
    ...
    if xmm != MARKER {
        println!("selftest: xmm0 lost across switches: {:#x}", xmm);
        return 25;
    }
    step!(22, kill(child));
    ...
    if w.reason != wait_reason::KILLED {
        println!("selftest: spin child ended with reason {}", w.reason);
        return 24;
    }
    println!("selftest: child spin killed after 50 ms (xmm0 preserved)");
    let _ = close(child);
    0
}
...
fn check_echo_child(root: Handle) -> i32 {
    ...
    println!("selftest: channel echoed {} bytes from the child", n);

    let w = step!(36, wait(&[child], 5_000_000_000));
    if w.reason != wait_reason::EXITED {
        println!("selftest: echo child ended with reason {}", w.reason);
        return 37;
    }
    let st = step!(38, proc_status(child));
    if st.kind != exit_kind::EXITED || st.code != 0 {
        println!("selftest: echo child exit kind {} code {}", st.kind, st.code);
        return 39;
    }
    let _ = close(a);
    let _ = close(b);
...
fn check_device_cap(root: Handle) -> i32 {
    if let Ok(info) = rondos_rt::info() {
        if info.fb_present != 0 && info.fb.phys != 0 {
            match rondos_rt::mem_map_phys(info.fb.phys, 4096, 0) {
                Ok((h, va)) => {
                    println!("selftest: framebuffer mapped at {:#x}", va);
                    let _ = rondos_rt::mem_unmap(h);
                }
                Err(e) => {
                    println!("selftest: framebuffer map failed: {:?}", e);
                    return 96;
                }
            }
        }
    ...
    if w.reason != wait_reason::EXITED {
        println!("selftest: physcheck ended with reason {}", w.reason);
        return 100;
    }
    let st = step!(101, proc_status(child));
    if st.kind != exit_kind::EXITED || st.code != 0 {
        println!("selftest: physcheck exit kind {} code {}", st.kind, st.code);
        return 102;
    }
    println!("selftest: device capability enforced");
    let _ = close(child);
    0
}
...
fn check_heap_program(root: Handle) -> i32 {
    ...
    if w.reason != wait_reason::EXITED {
        println!("selftest: heap child ended with reason {}", w.reason);
        return 93;
    }
    let st = step!(94, proc_status(child));
    if st.kind != exit_kind::EXITED || st.code != 0 {
        println!("selftest: heap child exit kind {} code {}", st.kind, st.code);
        return 95;
    }
    println!("selftest: Rust heap program exited 0");
    let _ = close(child);
    0
}
...
fn check_tmpfs(root: Handle) -> i32 {
    ...
    step!(65, close(h));
    if &buf[..n] != msg {
        println!("selftest: tmpfs read back {} bytes, expected {}", n, msg.len());
        return 66;
    }
    println!("selftest: tmpfs file round-trips ({} bytes)", n);
    ...
    println!("selftest: readdir found {} entries", count);
    if count < 6 {
        return 67;
    }
    0
...
fn check_p2_rest(root: Handle) -> i32 {
    ...
    if n != 3 || nh != 1 || buf[..3] != *b"mem" {
        println!("selftest: handle message malformed ({} bytes, {} handles)", n, nh);
        return 74;
    }
    let st = step!(75, rondos_rt::stat(got[0]));
    if st.va == 0 || st.va == va {
        println!("selftest: shared object not remapped (old {:#x}, new {:#x})", va, st.va);
        return 76;
    }
    let v = unsafe { (st.va as *const u64).read_volatile() };
    if v != 0x5150_5150_5150_5150 {
        println!("selftest: shared contents lost: {:#x}", v);
        return 77;
    }
    println!("selftest: handle passing ok (remapped {:#x} -> {:#x})", va, st.va);
    let _ = close(got[0]);
    let _ = close(mem);
    let _ = close(a);
    let _ = close(b);
    ...
    if pos != 4 {
        println!("selftest: seek returned {}", pos);
        return 83;
    }
    let mut b2 = [0u8; 6];
    let n = step!(84, rondos_rt::read(h, &mut b2));
    if &b2[..n] != b"456789" {
        println!("selftest: seek/read got {:?}", core::str::from_utf8(&b2[..n]));
        return 85;
    }
    step!(86, close(h));
    println!("selftest: seek ok ({} bytes after offset 4)", n);
    ...
    match rondos_rt::read(h, &mut stale) {
        Ok(_) => {
            println!("selftest: stale handle still readable after unlink");
            return 90;
        }
        Err(e) if e == rondos_abi::Status::BadHandle => {}
        Err(e) => {
            println!("selftest: stale handle gave {:?}, expected BadHandle", e);
            return 91;
        }
    }
    if open_file(root, path).is_ok() {
        println!("selftest: unlink did not remove the file");
        return 92;
    }
    println!("selftest: unlink ok (stale handle rejected)");
    ...
    if open_file_flags(root, b"/bin/init", shadow).is_ok() {
        println!("selftest: CREATE shadowed a boot-tar file");
        return 93;
    }
    println!("selftest: CREATE refuses to shadow the boot tar");
    0
}
...
fn check_c_program(root: Handle) -> i32 {
    ...
    if w.reason != wait_reason::EXITED {
        println!("selftest: chello ended with reason {}", w.reason);
        return 53;
    }
    let st = step!(54, proc_status(child));
    if st.kind != exit_kind::EXITED || st.code != 0 {
        println!("selftest: chello exit kind {} code {}", st.kind, st.code);
        return 55;
    }
    println!("selftest: C program exited 0");
    let _ = close(child);
    0
}
...
fn check_shared_memory() -> i32 {
    ...
    if va == 0 {
        println!("selftest: mem_map returned no address");
        return 41;
    }
    let p = va as *mut u64;
    unsafe {
        ...
        if p.read_volatile() != 0xfeed_face_cafe_1234 {
            println!("selftest: shared memory did not round-trip");
            return 42;
        }
    }
    println!("selftest: shared memory at {:#x} round-trips", va);
    let _ = rondos_rt::mem_unmap(h);
    0
}
...
pub extern "C" fn app_main(block: &StartupBlock) -> i32 {
    print!("selftest: hello from ring 3\n");
    unsafe { COUNTER += 1 };
    println!(
    ...
    let Some(root) = block.cap(ObjKind::Dir).map(|c| Handle(c.handle)) else {
        println!("selftest: no root capability in the StartupBlock");
        return 1;
    };
    println!("selftest: root capability handle {:#x}", root.0);
    ...
    println!("selftest: all children reaped, exiting");
    0
}
